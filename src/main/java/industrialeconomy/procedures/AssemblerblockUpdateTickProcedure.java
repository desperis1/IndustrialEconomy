	...
	public static void executeProcedure(Map<String, Object> dependencies) {
		...
		if (dependencies.get("y") == null) {
			if (!dependencies.containsKey("y"))
				IndustrialEconomyMod.LOGGER.warn("Failed to load dependency y for procedure AssemblerblockUpdateTick!");
			return;
		}
		...
		playerConfig = (File) new File((FMLPaths.GAMEDIR.get().toString() + "/config/"), File.separator + (owner + ".json"));
		{
			try {
				...
				if (200 < mainObject.get("Energy").getAsDouble()) {
					...
					}.getItemStack(new BlockPos((int) x, (int) y, (int) z), (int) (2))).getItem() == HeavyironblockBlock.block.asItem()
							&& new Object() {
								public int getAmount(IWorld world, BlockPos pos, int sltid) {
									AtomicInteger _retval = new AtomicInteger(0);
									TileEntity _ent = world.getTileEntity(pos);
