...
public class JetpackKeyBindKeyBinding extends IndustrialEconomyModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	...
	public void initElements() {
		keys = new KeyBinding("key.industrial_economy.jetpack_key_bind", GLFW.GLFW_KEY_SPACE, "key.categories.movement");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					...
					IndustrialEconomyMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(1, dt));
